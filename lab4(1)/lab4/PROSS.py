#!/usr/bin/env python3
"""
Calculate all torsion angles and assign secondary structure and mesostate code.
...
Date: September 2004 (Updated for Python 3 in 2024)
...
# This script does not require Numeric/Numpy
HAVE_NUMPY = 0    
...
pat_helix  = "(%s){5,}" % '|'.join(map(lambda x: "(%s)" % x, helix))
pat_strand = "(%s){3,}" % '|'.join(map(lambda x: "(%s)" % x, strand))
...
def _rc_find(codes, pattern, mcodes=None):
    ...
    try:
        while 1:
            mat = next(it)
            matches.append((int(mat.start()/CODE_LENGTH), int(mat.end()/CODE_LENGTH)))
...
class TypedList:
    ...
    def _alltrue(self, els):
        return len(els) == len(list(filter(None, map(self._func, els))))
...
def write_pdb(myMol, f, renum=0, seq=None, pack=pack_pdb_line):
    ...
    if hasattr(f, 'write'):
        ...
        import gzip
        out = gzip.open(f, 'wt') # Write Text mode for Py3
        close = 1
    else:
        out = open(f, 'w')
        default_seq = 0
...
def unpack_pdb_line(line, ATOF=_atof, ATOI=_atoi):
...
def read_pdb(f, as_protein=0, as_rna=0, as_dna=0, all_models=0,
    ...
    if type(f) is type(''):
        close = 1
        if f[-3:] == '.gz':
            inp = gzip.open(f, 'rt') # Text mode for Py3
        else:
            inp = open(f, 'r')
    else:
        inp = f
    ...
    try:
        ...
        # Fallback if inp.name is not available or if it's a GzipFile without name attr exposed directly
        sequences = {} 
        if hasattr(inp, 'filename'):
...
#            return 999.99
    ...
    def torsions(self, i=None, map=map):
        if not i is None:
            ...
            # Force list conversion here because in Py3 map returns an iterator
...
class PDBFile:
    ...
    def _openfile(self):
        if self.filename[-3:] == '.gz':
            self.file = gzip.open(self.filename, 'rt') # Text mode for Py3
        else:
